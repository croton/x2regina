...
*/
parse arg params
if params='-?' then do; 'MSG chooser [--c cmd | --f file | wordlist][--multi][--q]'; exit; end
paramsUC=translate(params)

select
  when pos('--C', paramsUC)>0 then do
    parse var params '--c' mycmd '--'
    call initListFromCmd mycmd
  end
  when pos('--F', paramsUC)>0 then do
...
ctr=0
maxlen=0
ADDRESS SYSTEM command '|RXQUEUE'
do while queued()<>0
  parse pull entry
...
cmds.0=0
if items='' then do; call help; exit; end
maxlen=0
count=words(items)
do i=1 to count
  cmds.i=word(items, i)
